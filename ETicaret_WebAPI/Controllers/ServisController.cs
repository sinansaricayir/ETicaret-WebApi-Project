...
namespace ETicaret_WebAPI.Controllers
{
    ...
    {
        ETicaretDbEntities2 db = new ETicaretDbEntities2();
        ...
        [HttpPost]
        [Route("api/kayitekle")]
        public SonucModel KayitEkle(Urun model)
        {
            if (db.Urun.Count(s => s.urunKatId == model.urunKatId && s.urunAd == model.urunAd) > 0)
            {
                sonuc.islem = false;
        ...
        {
            Urun kayit = db.Urun.Where(s => s.urunId == kayitId).SingleOrDefault();
            if (kayit == null)
            {
                sonuc.islem = false;
                sonuc.mesaj = "Kayıt bulunamadı.";
        ...
        [HttpPost]
        [Route("api/sepetekle")]
        public SonucModel SepetEkle(Sepet2Model model)
        {
            //aynı urun varsa ekletme
                       
            Sepet2 yeni = new Sepet2();
            ...
            sonuc.islem = true;
            sonuc.mesaj = "Ürün sepete eklenmiştir.";
            return sonuc;
        }
        ...
        public SonucModel YoneticiEkle(YoneticiModel model)
        {
            ...
            yeni.yoneticiYetki = model.yoneticiYetki;
        ...
        public SonucModel YoneticiDuzenle(YoneticiModel model)
        {
            ...
            kayit.yoneticiYetki = model.yoneticiYetki;
            db.SaveChanges();
            sonuc.islem = true;
        ...
        public YoneticiModel GirisYap(string mail, string sifre)
        {
            YoneticiModel uyemodel = db.Yonetici.Where(s => s.yoneticiAd == mail && s.yoneticiSifre == sifre).Select(x => new YoneticiModel()
            {
                yoneticiAd = x.yoneticiAd,
                yoneticiSifre = x.yoneticiSifre,
                yoneticiId = x.yoneticiId,
